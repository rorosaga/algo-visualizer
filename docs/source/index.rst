...
2. An abstract visualizer that can be used to visualize any algorithm that can be represented as a sequence of states. The states are captured using a lambda function that returns the current state of the algorithm at each iteration.
...
There will be two executables in the build directory: `main_visualizer` and `main_abstract`. The first one is the complete visualizer UI, and the second one is the abstract visualizer.
...
To run the abstract visualizer, execute the following command:

.. code-block:: bash

	./build/bin/main_abstract
